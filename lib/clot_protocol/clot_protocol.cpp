...
*/
/*
const unsigned short clot_Crc16Table[256] = {
...
uint16_t clot_crc16(uint8_t * pcBlock, uint16_t len)
{
    ...
    while (len--)
        ...
        crc = (crc << 8) ^ clot_Crc16Table[(crc >> 8) ^ *pcBlock++];
    return crc;
}
*/
...
)
{
    ...
    uint8_t START_BYTE = clot_master_start_byte;
    uint8_t SLAVE_ADDR = SLAVE_ADDR_;
    uint8_t UNIT_ADDR = UNIT_ADDR_;
    uint8_t COMMAND = COMMAND_;
   // uint8_t CRC_16_1;
   // uint8_t CRC_16_2;

    // lets calculate crc 16 sum of fields
    uint8_t array_to_calculate [clot_master_message_len - 2] = {
        START_BYTE,
        SLAVE_ADDR,
        UNIT_ADDR,
        COMMAND
    };
    uint16_t crc16 = clot_crc16(array_to_calculate, clot_master_message_len - 2);
